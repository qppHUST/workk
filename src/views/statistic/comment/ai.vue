...
import { getShortTextData } from '@/api/statistic';
...
export default {
  ...
  data() {
    ...
    const end_time = start_time.clone().subtract(7, 'days');
    return {
      tabList: [
        {
          key: 'aigc_gateway',
  ...
  methods: {
    ...
    getShortText() {
      ...
      this.loading = true;
      getShortTextData({ biz, user_id, start_time, end_time, type, page: page - 1, limit }).then(res => {
        const data = res.data || {};
