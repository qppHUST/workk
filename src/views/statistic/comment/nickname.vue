      ...
      <div class="flex flex-wrap mb20" >
        <a-form ref="formRef" :model="formState" layout="inline">
          <a-form-item ref="name" label="时间" name="name">
            <a-date-picker   v-model="formState.time" :format="dateFormat" :valueFormat="'X'"    />
...
import { getReportDayData } from '@/api/statistic';
...
export default {
  ...
  methods: {
    ...
    getReportDay() {
      ...
      this.loading = true;
      getReportDayData({ biz, user_id, start_time, end_time, type, page: page - 1, limit }).then(res => {
        const data = res.data || {};
    ...
    search() {
      ...
      this.filterList.biz = this.key
      this.filterList.start_time = Number(this.formState.time);
      this.filterList.end_time = Number(this.formState.time);
      this.getReportDay();
    },
    //导出
    handleExport() {
