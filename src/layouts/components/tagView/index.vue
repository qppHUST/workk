<template>
  <div class="tagView-wrapper relative white">
    ...
    <context-menu :visible.sync="menuVisible" @select="onMenuSelect" />
    <div class="scroll-outer" ref="scrollOuter" :style="outerWidth" @mousewheel="handleMouseWheel">
      <div class="scroll-body absolute" ref="scrollBody" :style="{ left: tagBodyLeft + 'px' }">
        <router-link
          ...
          ref="tagWrapper"
        >
          <span class="tag-title" @contextmenu="e => onContextmenu(item.path, e)">{{ item.meta.title }}</span>
          <svg-icon
            icon="close"
            v-if="index != 0"
...
export default {
  ...
  methods: {
    ...
    closeLeft(path) {
      this.$store.dispatch('tagsView/clearLeft', path)
      console.log('关闭左侧', path);
    },
    closeRight(path) {
       this.$store.dispatch('tagsView/clearRight', path)
      console.log('关闭右侧', pageKey);
    },
    closeOthers(path) {
      this.$store.dispatch('tagsView/clearOther', path)
      console.log('关闭其他', pageKey);
    },
    refresh(path) {
      console.log('刷新', pageKey);
    },
    isActive(path) {
